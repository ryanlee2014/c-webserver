...
int addltem(hashtable* table,char* key,content* cont,long CS)
{
...

{
	...
	(void)sprintf(log,"%s: %d",s1,i);



	if((fd = open("aaaaa.time",O_CREAT|O_WRONLY|O_APPEND,0644))>=0)
...
*/
		struct timeval tval_begin,tval_end,tval_all;
		long filesize,CS,time;
		...
		if((getContentByKey(table,&buffer[5] ) == NULL))
		{
			...
			timersub(&tval_end,&tval_begin,&tval_all);
			CS = (long)(tval_all.tv_usec/(unsigned long)get_file_size(&buffer[5]));
			length = len;
			content* cont3 = (content*)malloc(sizeof(content));
			cont3->length = length;
			cont3->address = contt1;
			addltem(table,&buffer[5],cont3,CS);
